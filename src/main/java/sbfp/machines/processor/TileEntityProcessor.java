...
public abstract class TileEntityProcessor extends TileEntity implements IUpdatePlayerListBox {
    ...
    protected void process() {
        ...
        try {
            if (!this.hasItem) {
                if (this.feedAndDryMergeOutputs()) {
                    this.hasItem = true;
                }
            }
    ...
    /**
     * Does a dry merge (sub-call to your container as you see fit.) Then
     * decreases inputs, if possible and they match, and sets both waitingOutput
     * and activeRecipe.
     *
     * @return The recipe to be used, or null if EITHER there's no room for the
     * outputs OR no inputs match. (Both need to be checked here; they're both
     * machine-dependent.)
     */
    ...
    public void writeToNBT(NBTTagCompound tagCompound) {
        ...
        if (this.activeRecipe != null) {
            tagCompound.setString("recipeID", this.activeRecipe.getName());
        }
        NBTTagList tagList = new NBTTagList();
        if (this.waitingOutputs == null) {
            return;
        }
