...
class LargeVeinGenerator {
    ...
    private void setBlock(int x, int y, int z) {
        ...
        if ((GeneratorOres.getBlock(world, newPosition) == Blocks.stone || this.dirtFlag && GeneratorOres.getBlock(world, newPosition) == Blocks.dirt) && rand.nextDouble() < Math.pow(Math.pow(dx, 16) + Math.pow(dy, 16) + Math.pow(dz, 16) + 1, -this.veinSize)) {
