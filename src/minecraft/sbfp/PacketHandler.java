...
public class PacketHandler implements IPacketHandler, IPacketReceiver{
	...
	*/
	...
	{
		return getPacketWithID(channelName, PacketType.UNKNOWN.ordinal(), sendData);
	}
	...
	*/
	...
	public static DataOutputStream encodeDataStream(DataOutputStream data, Object... sendData)
		...
		try
		{
			...
			{
				if (dataValue instanceof Integer) data.writeInt((Integer) dataValue);
				
				else if (dataValue instanceof Float) data.writeFloat((Float) dataValue);
				
				else if (dataValue instanceof Double) data.writeDouble((Double) dataValue);

				else if (dataValue instanceof Byte) data.writeByte((Byte) dataValue);
				
				else if (dataValue instanceof Boolean) data.writeBoolean((Boolean) dataValue);
				
				else if (dataValue instanceof String) data.writeUTF((String) dataValue);
				
				else if (dataValue instanceof Short) data.writeShort((Short) dataValue);
				
				else if (dataValue instanceof Long) data.writeLong((Long) dataValue);
				
				else if (dataValue instanceof NBTTagCompound) writeNBTTagCompound((NBTTagCompound) dataValue, data);
			}
