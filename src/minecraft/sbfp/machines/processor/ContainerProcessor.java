...
public abstract class ContainerProcessor extends Container{
	...
	public int dryMerge(ItemStack stack, int start, int end, boolean checkValidity){
		...
		for(int i = start; i<end; i++){
			...
			}else if(slotstack!=null&&slotstack.itemID==stack.itemID&&(!stack.getHasSubtypes()||stack.getItemDamage()==slotstack.getItemDamage())&&ItemStack.areItemStackTagsEqual(stack,slotstack)&&(!checkValidity||slot.isItemValid(stack))){
